#
# Prawn::Extensions::Markdown::Parser is extracted and adapted from https://github.com/AzionCo/markdown_prawn
# for now, it proposes a naive solution of rendering markdown-like style on a pdf
#
module Prawn::Extensions::Markdown::Parser
  ...
  class Parser
    ...
    def initialize(document_structure = [])
      @links_list = { :urls_seen => [], :object => Prawn::Extensions::Markdown::Fragments::LinksReferenceFragment.new }
      @document_structure = []
      @images_list = []
    end
    ...
    def parse
      paragraph = Prawn::Extensions::Markdown::Fragments::ParagraphFragment.new
      list = Prawn::Extensions::Markdown::Fragments::ListFragment.new
      in_list = false
      @content.each_with_index do |line, index|
        ...
        unless /^(#+)(\s?)\S/.match(line).nil?
          ...
          hashes = $1.dup
          heading = Prawn::Extensions::Markdown::Fragments::HeadingFragment.new([line.gsub(hashes,'')])
        ...
        if !/^(=)+$/.match(line).nil?
          ...
          heading = Prawn::Extensions::Markdown::Fragments::HeadingFragment.new([@content[index - 1]])
          heading.level = 1
          @document_structure << heading
        end
        ...
        if !/^(-)+$/.match(line).nil?
          if @content[index - 1].strip == ''
            ...
            #
            paragraph.content = paragraph.content.delete_if { |i| i == line }
            @document_structure << Prawn::Extensions::Markdown::Fragments::HorizontalRuleFragment.new
          else
            paragraph.content = paragraph.content.delete_if do |item|
              item == line || item == @content[index - 1]
            end
        ...
        line.scan(/(?:^|\s)?(\!\[(?:.+?)\]\((.+?)\))/) do |val|
          ...
          to_replace << val[0]
          @document_structure << Prawn::Extensions::Markdown::Fragments::ImageFragment.new([val[1]])
        end
        ...
        line.scan(/(?:^|\s)?(\[(?:.+?)\]\((.+?)\))/) do |val|
          if @links_list[:urls_seen].include?(val[1])
            ...
            @links_list[:object].content <<  [ @links_list[:object].content.size, val[1], "#{title}" ]
          end
          _title = paragraph.content[-1].scan(/\[(.*?)\]/).flatten.first || '<unknow>'
